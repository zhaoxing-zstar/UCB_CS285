...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        loss = -torch.mul(action_dist.log_prob(actions),advantages).sum()
        loss /= observations.shape[0]
        ...
        train_log = {
            'Policy Network Loss': ptu.to_numpy(loss),
        }
        ...
        if self.nn_baseline:
            ...
            q_values = utils.normalize(q_values, np.mean(q_values, axis=0, keepdims=True), np.std(q_values, axis=0, keepdims=True))
            q_values = ptu.from_numpy(q_values)
            pred_baseline = self.baseline(observations)
            baseline_loss = self.baseline_loss(pred_baseline, q_values).sum()
            baseline_loss /= observations.shape[0]

            self.baseline_optimizer.zero_grad()
            baseline_loss.backward()
            self.baseline_optimizer.step()
            train_log.update({'Baseline Loss': ptu.to_numpy(baseline_loss)})
        return train_log
