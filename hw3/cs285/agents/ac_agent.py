...
class ACAgent(BaseAgent):
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        adavntage = self.estimate_advantage(ob_no, next_ob_no, re_n, terminal_n)
        for _ in range(self.agent_params['num_actor_updates_per_agent_update']):
            actor_loss += self.actor.update(ob_no, ac_na, adavntage)
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        v_s_prime = self.critic.forward_np(next_ob_no) * (1-terminal_n)
        qa_s_curr = re_n + self.gamma * v_s_prime
        adv_n = qa_s_curr - v_s
